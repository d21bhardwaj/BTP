...
% simulation
Run = 1000000;
%Observation Cell stores the Result of each Simulation%
...
%Table1 and Table 2 for better visualization of the result
for i = 1:3
    Table2{2+i,1}=i;
    for j = 1:3
        Table1(3*(i-1)+j, 1)=i;
...
for i=1:Run
    %Generate a random instruction set Instead of 1 to 6, 0 to 7 can also be
    %used if we are considering RRR and GGG to be the instruction set as well
     y = (randi([0 7],1));
    ...
    str = string(y(1,1)); str = append(str,string(y(1,2)));
    ...
    Table2{A_C+1,2}= Table2{A_C+1,2}+1; Table2{B_C+1,3}= Table2{B_C+1,3}+1;
    Table2{A+2,2}= Table2{A+2,2}+1; Table2{B+2,3}= Table2{B+2,3}+1;
    ...
    %temp will be 12RG
    temp = string(A);temp = append(temp,string(B));
    if(A_C)
        temp = append(temp,'G');
    else 
        temp = append(temp,'R');
